...
import yaml
from local_features.sift import SIFT
from local_features.superpoint import SuperPoint
...
def init_model(config, benchmark_name, root_dir='.'):
    ...
    if class_name == 'SIFT':
        model = SIFT(model_conf)
    elif class_name == 'SuperPoint':
        model = SuperPoint(model_conf)
    else:
